...
var C99CommentBackslashRegex, C99CommentNoBackslashRegex, ConcatBackslashNewlinesStream, addDefine, addFunctionMacro, addObjectMacro, allowedConditionalChars, analyzeLines, applyDefines, applyFunctionDefine, argumentRegex, backslashNewlineRegex, charInQuotesRegex, condRegex, defineErrorCol, dir, directiveRegex, doIfCondMath, err, fileTokenRegex, fs, gccVersionDirs, getBackslashNewlinesBeforeToken, hashRegex, ifDefinedCallback, insertInclude, j, leadingWhitespaceRegex, len, lineTokenRegex, localHeaderRegex, localIncludeDirs, notWhitespaceRegex, numberRegex, parenCommaWhitespaceRegex, parentheticalExprRegex, path, processComments, processError, processIf, processIfConstExpr, processLine, processLineDirective, processPragma, processSourceLine, removeDefine, res, slashStarBeginRegex, slashStarEndRegex, stream, stringInQuotes, stripQuotesRegex, stripSideCaratsRegex, sysIncludeDirs, systemHeaderRegex, throwError, tokenRegex, trailingWhitespaceRegex, whitespaceRegex,
...
gccVersionDirs = (fs.readdirSync("/usr/lib/gcc")).map(function(inode) {
...
throwError = function(file, line, line_num, col, err) {
  ...
  for (i = k = 1, ref = col - 1; k <= ref; i = k += 1) {
...
getBackslashNewlinesBeforeToken = function(str, tok) {
  ...
  prevChar = "";
  num = 0;
  for (i = k = 0, ref = ind - 1; k <= ref; i = k += 1) {
    if (str.charAt(i) === "\n" && prevChar === "\\") {
      ++num;
    }
...
applyFunctionDefine = function(fnDefn, args, token, str, defines, macrosExpanded, opts, line) {
  ...
  curStr = fnDefn.text;
  for (i = k = 0, ref = args.length - 1; k <= ref; i = k += 1) {
...
applyDefines = function(str, defines, opts, macrosExpanded, line) {
  var argInP, argsArr, argsInParens, defineFunctionRegexStr, defineObjectRegexStr, defineStr, defineVal, definesToSend, i, k, l, len1, ref, replaceString, tkwArgs, tokensWithArgs;
    ...
    if ((!macrosExpanded) || (macrosExpanded.indexOf(defineStr) === -1)) {
      ...
      if (defineVal.type === "object" && str.match(new RegExp(defineObjectRegexStr, "g"))) {
        ...
        argsInParens = (function() {
          var k, len1, results;
          results = [];
          for (k = 0, len1 = tokensWithArgs.length; k < len1; k++) {
            tkwArgs = tokensWithArgs[k];
        ...
        for (i = l = 0, ref = tokensWithArgs.length - 1; l <= ref; i = l += 1) {
...
insertInclude = function(directive, restOfLine, outStream, opts, dirname, line) {
  var filePath, found, includeDir, k, len1, localHeader, newPipe, prevFile, prevInFileStream, prevLine, ref, ref1, sysHeader;
  ...
  if (sysHeader) {
    ...
    for (k = 0, len1 = sysIncludeDirs.length; k < len1; k++) {
      includeDir = sysIncludeDirs[k];
      try {
...
doIfCondMath = function(str, opts, line) {
  ...
  if (res) {
    return throwError(opts.file, line, opts.line, defineErrorCol, "invalid character in preprocessor conditional: " + res[0]);
  } else {
    try {
      resVal = eval(str);
    } catch (_error) {
...
analyzeLines = function(file, fileStream, opts) {
  ...
  lineStream.on('line', function(line) {
    return processLine(line, outStream, opts, inComment, dirname);
  });
