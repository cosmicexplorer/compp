...
var C99CommentBackslashRegex, C99CommentNoBackslashRegex, CFormatStream, ConcatBackslashNewlinesStream, addDefine, addFunctionMacro, addObjectMacro, analyzeLines, applyDefines, applyFunctionDefine, argumentRegex, backslashNewlineRegex, charInQuotesRegex, condRegex, defineErrorCol, dir, directiveRegex, disallowedConditionalChars, doIfCondMath, fileTokenRegex, fs, gccADir, gccArchDirs, gccVersionDirs, getBackslashNewlinesBeforeToken, hashRegex, ifDefinedCallback, insertInclude, leadingWhitespaceRegex, lineTokenRegex, localHeaderRegex, localIncludeDirs, multipleWhitespaceRegex, notWhitespaceRegex, numberRegex, parenCommaWhitespaceRegex, parentheticalExprRegex, path, processComments, processError, processIf, processIfConstExpr, processLine, processLineDirective, processPragma, processSourceLine, removeDefine, slashStarBeginRegex, slashStarEndRegex, stringInQuotes, stripQuotesRegex, stripSideCaratsRegex, sysIncludeDirs, systemHeaderRegex, throwError, tokenRegex, trailingWhitespaceRegex, whitespaceRegex,
...
insertInclude = function(directive, restOfLine, outStream, opts, dirname, line) {
  var err, filePath, found, headerArr, headerFilename, includeDir, j, len, localHeader, newPipe, prevCallback, prevFile, prevLine, prevLineStream, ref, ref1, res, sysHeader;
  ...
  found = false;
  if (sysHeader) {
    headerFilename = sysHeader.replace(leadingWhitespaceRegex, "").replace(trailingWhitespaceRegex, "").replace(stripSideCaratsRegex, "");
    headerArr = sysIncludeDirs;
  } else if (localHeader) {
    headerFilename = localHeader.replace(leadingWhitespaceRegex, "").replace(trailingWhitespaceRegex, "").replace(stripQuotesRegex, "");
    headerArr = opts.includes;
  } else {
    throwError(opts.file, line, opts.line, defineErrorCol, "#include without header");
  }
  for (j = 0, len = headerArr.length; j < len; j++) {
    ...
    try {
      ...
      newPipe.removeAllListeners('end');
  ...
  if (!found) {
    return throwError(opts.file, line, opts.line, defineErrorCol, "Include file " + headerFilename + " not found.");
  }
};
...
analyzeLines = function(file, fileStream, opts) {
  var dirname, inComment, lineStream, outStream;
  opts.line = 1;
  opts.file = file;
  opts.isInComment = false;
  ...
  opts.dataCallback = function(chunk) {
